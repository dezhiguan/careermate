...
import com.careermate.common.support.RelativeDates;
...
public class CreateCareerTaskTool implements AgentTool {
     ...
     * 解析截止日期，容忍用户口语。
     *
     * <p>用户说的是「周五前补完」，意图路由就把「周五」原样传进来，{@code LocalDate.parse}
     * 直接抛异常让整个建任务动作失败。截止日期本是选填项，不该因为一个说法不标准就让主动作失败——
     * 认得出就折算成具体日期，认不出就当作没填，任务照建。
     */
    private LocalDate parseDueDate(Object value) {
        ...
        LocalDate parsed = RelativeDates.parse(text);
        if (parsed == null) {
