...
class InterviewQuestionPromptsTest {
    ...
    void promptContainsAllContextsAndTagRules() {
        String prompt = InterviewQuestionPrompts.jdAwarePrompt(
                "字节-后端", "JD正文内容", "简历摘要内容", "题库参考内容", "字节面经内容");
        ...
        assertTrue(prompt.contains("字节面经内容"));
        assertTrue(prompt.contains("该公司面经"));
        assertTrue(prompt.contains("JD_FOCUSED"));
        assertTrue(prompt.contains("WEAK_POINT"));
    }

    @Test
    void promptHandlesNullArgsAndBlankCompany() {
        String prompt = InterviewQuestionPrompts.jdAwarePrompt(null, null, null, null, null);
        assertTrue(prompt.contains("questions"));
        // 无公司面经时给占位说明
        assertTrue(prompt.contains("暂无该公司面经"));
    }
}
