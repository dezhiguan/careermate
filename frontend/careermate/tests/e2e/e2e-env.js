...
const MOCK_REPLY =
  /Mock 简历分析结果|我已读取你的默认简历|我已通过工具读取你的默认简历|我已读取你最近的岗位匹配结果|我已通过工具读取你最近的岗位匹配|我已为你生成岗位匹配结果|我已为你创建面试训练|我已读取你的求职看板|还没有读取到默认简历|还没有读取到岗位匹配记录|建议你突出项目中的业务指标|建议你重点优化|匹配分数|缺失技能|Agent Trace|流式完成|DONE|MESSAGE|我记得|Java 后端|基于你之前提到的目标/i;
