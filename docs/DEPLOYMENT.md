...
详细运维步骤见 **[deployment-careermate.md](deployment-careermate.md)**。SkyWalking 见 **[skywalking-cloud-setup.md](skywalking-cloud-setup.md)**。
...
|----|------|
| 入口 | `8.163.63.222`（示例） |
| 后端端口 | `18080`（避免与 RAGForge `8080` 冲突） |
| 前端路径 | `/careermate/` |
| API 路径 | `/careermate-api/` → 反代到 `...:18080/api/` |
| SkyWalking UI | `/skywalking/` → `127.0.0.1:18088`（勿公网裸露 8088） |
| 密钥 | 仅 `/opt/careermate/backend/.env.app`，**不入库** |
...
| `JAVA_TOOL_OPTIONS` | `-javaagent:.../skywalking-agent.jar` 等 |
